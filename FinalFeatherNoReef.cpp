...
const int OpenMS = 1600;
const int CloseMS = 1500;
...
// --- TESTING MODE: always read from LOG_00.csv, never create a new file ---
const char* TEST_LOG_FILE = "LOG_00.csv";
...
void setup() {
    ...
    if (!SD.begin(AdaloggerPin)) {
        ...
        // Testing mode: just confirm LOG_00.csv exists, don't create a new file
        if (SD.exists(TEST_LOG_FILE)) {
            Serial.print("Test log file found: ");
            Serial.println(TEST_LOG_FILE);
        } else {
            Serial.println("WARNING: LOG_00.csv not found on SD card!");
            Serial.println("Please create it manually with your dummy data.");
        }
    }
...
void loop() {
    switch (currentState) {
        ...
        case STATE_LISTEN:
            ...
            if (Serial1.available()) {
                ...
                if (cmd.indexOf("RETRACT") >= 0) {
                    Serial.println("Power cycled, retracting");
        ...
        case STATE_DEPLOY:
            ...
            Turn(DeployedTurns);
            delay(DeployedTurns * turnDelay);

            readReg(0x18);
            freespin();
            Serial.println("Closing Brake...");
            Brake.detach();
        ...
        case STATE_RETRACT:
            ...
            Serial.println("Sending RETRACTED confirmation...");
            sendAcoustic("RETRACTED");

            readReg(0x18);
            freespin();

            // Testing mode: do NOT call updateFileName() — stay on LOG_00.csv
    ...
    if (Serial2.available() > 0) {
        ...
        if (firstByte == 0xFF) {
            ...
            if (Serial2.available() >= 23) {
                uint8_t header[3];
                header[0] = Serial2.read();
                header[1] = Serial2.read();
                header[2] = Serial2.read();
                ...
                if (header[2] == 0xBB) {
                    ...
                    if (vals[4] == expectedChecksum) {
                        // Testing mode: still log to LOG_00.csv if real data arrives
                        File logFile = SD.open(TEST_LOG_FILE, FILE_WRITE);
                        if (logFile) {
                            logFile.print(currentCollectionTime);
                            logFile.print(",");
                            logFile.print(header[1]);
                            logFile.print(",");
                            logFile.print(vals[0], 2);
                            logFile.print(",");
                            logFile.print(vals[1], 2);
                            logFile.print(",");
                            logFile.print(vals[2], 2);
                            logFile.print(",");
                            logFile.print(vals[3], 2);
                            logFile.print(",");
                            logFile.print(vals[4], 2);
                            logFile.println();
                            logFile.close();
                            Serial.print(">>> Logged: ");
...
void dumpDataToAcoustic() {
    ...
    Serial.println("Reading from: " + String(TEST_LOG_FILE));
    ...
    File logFile = SD.open(TEST_LOG_FILE, FILE_READ);
    if (!logFile) {
    ...
    // Testing mode: do NOT delete or modify LOG_00.csv after dump
    logFile.close();
